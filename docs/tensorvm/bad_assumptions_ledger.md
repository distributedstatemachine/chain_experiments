...
| BA-023 | "A v2-shaped candidate implementation discharges the proof obligations." | Critical | The local v2-block path is build-clean and adds useful fields, strict vote validation, included-receipt exclusion, and validator proposer checks, but still lacks exact parent snapshots, semantic check leaves, difficulty economics, and fallback semantics. | Build-clean partial code can be useful evidence, but it cannot prove consensus soundness until every theorem gate is discharged. | The candidate is directionally aligned but not proof-sound. | Add adversarial tests and discharge each theorem gate; see `mvp_core_candidate_v2_block_audit.md`. |
...
| BA-026 | "A zero-receipt or timeout block is useful-verification PoW." | Critical | The v2 spec needs a PoW-skip fallback for zero-receipt or no-PoW periods, but current fallback evidence is v1/local and the local v2-block path has no fallback implementation. | Fallback preserves liveness when useful-PoW is unavailable; it does not prove verification work, cannot reuse normal useful-PoW rewards, and cannot skip parent-state validation. | Fallback is a separate liveness-only proof obligation. | Add a disjoint fallback block kind, timeout/no-work evidence, deterministic validator rotation, reduced rewards, no miner TWU rewards, and parent-state validation; see `mvp_core_fallback_liveness_model.md`. |
