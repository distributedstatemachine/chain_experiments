...
| AC13 public evidence validator | [`public_testnet_evidence.md`](public_testnet_evidence.md), `parse_public_testnet_evidence_manifest`, `PublicTestnetEvidenceBundle`, external publication URI validation, verified manifest publication signatures, signed block/finality/data-availability summary roots, operator-attestation-derived external-operator evidence, and `tvmd public-evidence validate --manifest <path>` | Present |
...
verify manifest publication signatures, signed block/finality/data-availability summary roots, and
external-operator evidence derived from manifest operator-attestation records. These blockers therefore
require real external infrastructure rather than loopback, private-network, unsigned, or out-of-band
manifests.
