...
| Local CPU Docker Compose testnet | [`local_cpu_testnet_spec.md`](local_cpu_testnet_spec.md), `deploy/tensorvm/local-cpu/docker-compose.yml`, CPU-only `deploy/tensorvm/local-cpu/Dockerfile`, `deploy/tensorvm/local-cpu/scripts/entrypoint.sh`, `deploy/tensorvm/local-cpu/scripts/check-local-testnet.sh`, `deploy/tensorvm/local-cpu/scripts/check-restart-continuity.sh`, `deploy/tensorvm/local-cpu/scripts/check-rolling-restart-continuity.sh`, `local_cpu_compose::local_cpu_compose_bundle_matches_spec_artifact_shape`, `docker compose -f deploy/tensorvm/local-cpu/docker-compose.yml config --quiet`, `docker compose -f deploy/tensorvm/local-cpu/docker-compose.yml build`, `docker compose -f deploy/tensorvm/local-cpu/docker-compose.yml up --wait`, the local check script reporting 10 ready miners, 5 ready validators, 15 distinct operator IDs, 15 distinct libp2p peer IDs derived from stable operator seeds, 15 distinct node multiaddrs, 15 libp2p-ready nodes, 10 CPU-ready miners, zero CUDA-required miners, `runtime_command=miner_run` for miners, `runtime_command=validator_run` for validators, `validator-00` as the single local timed producer, miner block-production capability disabled, live role-loop counters, decoded network-event ingestion, decoded block, job, receipt, and attestation payload application, and network-applied block counters for every non-producer, real libp2p connected-peer counts, and observed job/receipt/attestation/block-payload gossip counters for every counted operator, 10 settled receipts, settled matmul and LinearTrainingStep work, positive rewarded miners, full finality/data-availability bps, seeded gateway `/chain/head` height/block count, host gateway reachability, standalone explorer readiness, browser explorer WebSocket polling through `/explorer/ws`, live post-startup advancement, named post-seed TensorOp and LinearTrainingStep receipt evidence, live tensor fetches, all-operator `tvmd service status`, all-operator finalized common-head checks and p2p-observed finalized checkpoint block/state-root checks through `tvmd service block`, all-operator block-log root reporting, local-only evidence flags, and a rolling restart-continuity matrix proving stable peer IDs, service-init recovery from torn snapshot/block-log state, advancing height/block-count/state-root/block-log-root evidence, preserved pre-restart finalized common head and state root, and continued finalization for every counted operator restart. Latest full checker rerun remains blocked at the bounded gateway `/health` probe; this iteration reran Compose config only. | Reference harness present; full checker blocked |
...
Current AC13 evidence tooling also includes `tvmd evidence run window-file ...`, which derives
...
| 14 | Zero-receipt epochs have a tested stake-weighted PoW-skip fallback path | Existing fallback proposer and liveness tests cover the superseded v1 proposer model | Not complete |
