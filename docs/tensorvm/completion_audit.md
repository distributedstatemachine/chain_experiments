...
| `cargo test --workspace --release` | Latest iteration evidence records 14 `pearl_chain`, 176 `tensor_vm` library tests, 2 `tvmd` binary tests, and 5 `tvmd` CLI integration tests | Passed |
| `cargo clippy --workspace --all-targets -- -D warnings` | Latest iteration evidence records pass from workspace root | Passed |
| `cargo tarpaulin` | [`tarpaulin_report.md`](tarpaulin_report.md) records 190 instrumented tests | Passed |
...
| CUDA feature gate | [`implementation_status.md`](implementation_status.md) records 180 `tensor_vm` tests under `--features cuda-kernels` | Passed locally |
