...
- the public evidence bundle must include a signed wall-clock run window, signed node heartbeats,
  ...
  network-observation records, signed external raw-record artifact locators, and signed deployed-service
  health records bound to external HTTPS URLs
...
external audit URIs, and external-operator evidence derived from signed manifest operator-attestation
records.
