...
| `cargo test --workspace --release` | Latest iteration evidence records 14 `experiments`, 199 `tensor_vm` library tests, 1 `tensor_vm_explorer` library test, 3 `tvmd` binary tests, 1 local CPU Compose integration test, and 6 `tvmd` CLI integration tests | Passed |
...
| TensorVM line coverage | [`tarpaulin_report.md`](tarpaulin_report.md) records 100.00% `tensor_vm` crate line coverage, 8873/8873 lines covered | Passed |
