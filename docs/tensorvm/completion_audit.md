...
| P2P/RPC runtime and socket tests | `p2p` rust-libp2p swarm, background libp2p service runtime, Kademlia/bootstrap, protocol tests, P2P codec tests, peer-book upsert and `/p2p/<peer-id>` bootstrap address loading, `rpc` socketed HTTP and health-route tests, `tvmd service init/peer add/serve` launch validation, `tvmd_cli::service_cli_lifecycle_starts_libp2p_and_serves_health_once`, public network-observation evidence rejected without nonzero TCP listen ports, and public evidence service URLs rejected when local, private, special-use DNS, single-label DNS, documentation, shared-address, benchmarking, multicast, reserved, or not exact query-free service paths | Reference implementation present |
