...
labels, invalid ports, localhost, `.local`, `.localhost`, `.test`, `.example`, `.invalid`, RFC example
domains, private, link-local, documentation, shared-address, benchmarking,
...
IDs, matching service-health HTTPS authorities, and required content paths, and external-operator evidence derived from signed manifest
