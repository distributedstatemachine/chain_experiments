...
| Reward finality and challenges | `TVM-REWARD-001`, `mvp_core_reward_finality_challenge_model.md` | Implementation-blocked | The paper model now separates finalized blocks from pending verifier-dependent rewards. | Pending/challenged/invalidated/settled reward state, consensus challenge openings, and clawback/nonpayment transitions are not implemented or tested. |
