...
| AD-010 | Canonical settled-receipt blockspace exists. | `implementation-dischargeable` | Current block roots commit global maps, not a selected eligible receipt set. | Add selected receipt pool metadata, deterministic ordering, caps, expiry, spent/carry-over rules, and root over selected leaves. | "Receipt roots commit state content, not v2 blockspace." |
| AD-011 | Block-level `checks_root` binds proposer verification work. | `implementation-dischargeable` | Per-attestation `checks_root` exists, but blocks do not aggregate selected receipt checks. | Define check leaves, aggregate root format, recomputation rules, and openings/challenges tied to selected receipts. | "Per-receipt checks roots are attestation evidence only." |
| AD-012 | Useful-verification PoW exists and dominates nonce grinding. | `implementation-dischargeable`, `permanent-assumption`, `evidence-dischargeable` | Current blocks lack nonce, target, selected receipt root, and checks root. | Add v2 PoW predicate and validation; measure verification work versus nonce grinding under chosen parameters; keep economic assumptions explicit. | "Useful-verification PoW is a target, not current behavior." |
...
| AD-015 | Zero-receipt or timeout fallback preserves consensus safety. | `implementation-dischargeable` | The reviewed fallback object is not implemented as a v2 transition. | Specify timeout, validator rotation, reduced rewards, no miner TWU rewards, and exact fallback validity predicate with tests. | "Fallback remains a missing v2 proof obligation." |
