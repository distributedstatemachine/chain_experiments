...
| AD-015A | Verifier-dependent reward finality waits for challenges or direct recomputation. | `implementation-dischargeable`, `formalizable`, `evidence-dischargeable` | The reward-finality state machine is specified in `mvp_core_reward_finality_challenge_model.md`, but current/candidate consensus state does not discharge pending claims, challenge resolution, or clawback. | Add pending/challenged/invalidated/settled reward states, challenge openings, deterministic resolution, DA-through-window evidence, and settlement tests. | "Block finality may create pending reward claims, not immediately spendable rewards." |
