...
- Public 7-day independent-node testnet evidence is not available in this repository; typed evidence
  ...
  signed external artifact locators for raw supporting records, disjoint miner/validator operator IDs,
  auditor IDs distinct from the manifest signer with auditor observations at or after the signed run end,
  ...
  URI path enforcement with query and fragment rejection, exact service URL path matching with query and
  fragment rejection, no overreported operator-attestation counts,
  ...
  single-label DNS multiaddrs, service health/content evidence must use matching HTTPS authorities for each
