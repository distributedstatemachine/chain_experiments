...
- Public 7-day independent-node testnet evidence is not available in this repository; typed evidence
  ...
  counters, non-public IP literal rejection for public endpoints, well-formed `ipfs://`/`ar://` identifier
  validation, and deployed public-service reachability plus content roots bound to external HTTPS URLs. A
  local launch preflight manifest is documented in
