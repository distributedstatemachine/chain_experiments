...
attestation, and block gossip counters, decoded network-event ingestion counters, the latest observed
block height/hash, and the bounded observed block-hash set so the first gate proves every operator is
connected and receiving live consensus announcements for the convergence target, not merely configured.
