...
fn local_cpu_compose_bundle_matches_spec_artifact_shape() {
    ...
    assert_shell_logical_lines(
        ...
        &[
            ...
            r#"tvmd role miner register --stake "$MINER_STAKE" > "$DATA_DIR/role-register.out""#,
            r#"tvmd role miner check --wallet "$WALLET" --device cpu --node "$NODE_MULTIADDR" > "$DATA_DIR/role-start.out""#,
            r#"tvmd role validator register --stake "$VALIDATOR_STAKE" > "$DATA_DIR/role-register.out""#,
            r#"tvmd role validator check --wallet "$WALLET" --node "$NODE_MULTIADDR" > "$DATA_DIR/role-start.out""#,
            ...
            r#"exec tvmd role proposer run --wallet "$WALLET" --node "$NODE_MULTIADDR" --listen "$RPC_LISTEN" --p2p-listen "$P2P_LISTEN" --data-dir "$DATA_DIR" --identity-seed "$IDENTITY_SEED" --auth-token "$AUTH_TOKEN" --max-requests 0"#,
            r#"exec tvmd role miner run --wallet "$WALLET" --device cpu --node "$NODE_MULTIADDR" --listen "$RPC_LISTEN" --p2p-listen "$P2P_LISTEN" --data-dir "$DATA_DIR" --identity-seed "$IDENTITY_SEED" --auth-token "$AUTH_TOKEN" --max-requests 0"#,
            r#"exec tvmd role validator run --wallet "$WALLET" --node "$NODE_MULTIADDR" --listen "$RPC_LISTEN" --p2p-listen "$P2P_LISTEN" --data-dir "$DATA_DIR" --identity-seed "$IDENTITY_SEED" --auth-token "$AUTH_TOKEN" --max-requests 0"#,
        ],
    );
