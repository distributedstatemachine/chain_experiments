//! Prime-field arithmetic for deterministic TensorVM consensus semantics.
//!
//! The modulus is the Mersenne prime 2^31 - 1. It keeps products inside u64 and
//! lets tensor kernels accumulate dot products in u128 with one reduction per
//! cell.
...
mod tests {
    ...
    fn field_wraps_correctly() {
        ...
        assert_eq!(normalize(MODULUS + 7), 7);
    }
}
