//! Minimal SHA-256 implementation for TensorVM hashes and random oracles.
...
impl Sha256 {
    ...
    pub fn update_len_prefixed(&mut self, value: &[u8]) {
    ...
    fn process_block(&mut self, block: &[u8; 64]) {
        ...
        for (i, chunk) in block.chunks_exact(4).take(16).enumerate() {
            w[i] = u32::from_be_bytes(chunk.try_into().expect("slice length checked"));
        }
        for i in 16..64 {
...
mod tests {
    ...
    #[test]
    fn len_prefixed_hashing_separates_ambiguous_parts() {
        let mut left = Sha256::new();
        left.update_len_prefixed(b"ab");
        left.update_len_prefixed(b"c");

        let mut right = Sha256::new();
        right.update_len_prefixed(b"a");
        right.update_len_prefixed(b"bc");

        assert_ne!(left.finalize(), right.finalize());
    }

    #[test]
    fn default_hasher_and_u32_update_match_manual_bytes() {
        let mut via_default = Sha256::default();
        via_default.update_u32(0x1234_5678);

        let mut via_bytes = Sha256::new();
        via_bytes.update(&0x1234_5678_u32.to_le_bytes());

        assert_eq!(via_default.finalize(), via_bytes.finalize());
    }
}
