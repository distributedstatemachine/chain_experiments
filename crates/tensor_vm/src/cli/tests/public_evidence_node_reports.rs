...
fn execute_node_evidence_reports_outputs() {
    ...
    for (role, address_label, operator_label, tag) in node_cases {
        let node = execute_public_evidence_command(&EvidenceCommand::Node(
            EvidenceNodeCommand::Heartbeat(NodeHeartbeatArgs::new(
                public_node_identity_args(role, address_label, operator_label),
    ...
    let operator_attestation = execute_public_evidence_command(&EvidenceCommand::Node(
        EvidenceNodeCommand::OperatorAttestation(OperatorAttestationArgs::new(
            public_node_identity_args(PublicNodeRole::Miner, b"miner-a", b"miner-a-operator"),
            operator_identity_uri.clone(),
...
fn public_node_identity_args(
    ...
    PublicNodeIdentityArgs::new(
        node_role_arg(role),
        address_arg(address(address_label)),
        operator_id_args(hash_bytes(b"test", &[operator_label])),
    )
}
