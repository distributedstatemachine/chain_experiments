...
inside the signed run window. Every `record-summary` root must have a matching signed `record-artifact`
locator for the external raw-record artifact. Preserve raw supporting records for:
