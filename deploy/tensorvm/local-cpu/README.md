...
network-applied block counters, decoded network-event ingestion counters, decoded job-payload application
through the chain engine, real libp2p connected-peer counts, observed job, receipt, attestation, and block
gossip counters, latest observed block heights and hashes, the bounded observed block-hash set, and chain
